...
function Bag() {
  const { cartCount, cartDetails, formattedTotalPrice, ...rest } = useShoppingCart()
  ...
  const onSubmit = async () => {
    try {
      ...
      const response = await axios.post('/api/checkout', {
        items: cart.map(item => ({  quantity: item.quantity, price: item.price_id }))
      })
        ...
        <SideBar>
          <Content>
              ...
              <footer>
                ...
                <button onClick={onSubmit} disabled={!cartCount || isCreatingCheckoutSession}>
                  Finalizar Compra
                </button>
              </footer>
          </Content>
